package
{
   ...
   public class WORKERS
   {
      ...
      public static function Assign(param1:BFOUNDATION) : Object
      {
         ...
         {
            _loc5_ = _workers[_loc4_];
            if(!_loc5_.task)
            {
               _loc6_ = Point.distance(new Point(_loc5_.mc.x,_loc5_.mc.y),new Point(param1._mc.x,param1._mc.y));
               if(_loc6_ < _loc2_)
               {
                  _loc2_ = _loc6_;
                  _loc3_ = _loc5_;
         ...
         if(_loc3_)
         {
            ...
            if(!GLOBAL._catchup)
            {
               _loc7_ = _sayings.assign[int(Math.random() * _sayings.assign.length)];
