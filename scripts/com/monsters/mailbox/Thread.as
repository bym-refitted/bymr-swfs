package com.monsters.mailbox
{
   ...
   public class Thread extends Thread_CLIP
   {
      ...
      public function threadLoadComplete(param1:Event) : void
      {
         ...
         outline_mc.width = 5 * 60;
         largeOutline_mc.width = 5 * 60;
         msg_txt.width = 5 * 60;
         addChild(sendBtn);
         ...
         {
            _loc5_ = this.data.convo[this.data.convo.length - 1];
            _loc2_ = _loc5_.userid == LOGIN._playerID;
         }
         if(!_loc2_)
         {
         ...
         while(_loc4_ < this.data.convo.length)
         {
            ...
            if(!_loc6_)
            {
               _loc8_ = new ThreadMember();
               _loc8_.Setup(this.data.convo[_loc4_]);
      ...
      private function truceAccept(param1:MouseEvent) : void
      {
         ...
         _loc5_.load(GLOBAL._apiURL + "player/sendmessage",_loc4_,this.onTruceAcceptSuccess,this.onFail);
      ...
      private function truceReject(param1:MouseEvent) : void
      {
         ...
         var _loc5_:URLLoaderApi = new URLLoaderApi();
         _loc5_.load(GLOBAL._apiURL + "player/sendmessage",_loc4_,this.onTruceRejectSuccess,this.onFail);
         this.sending = true;
         this.Validate();
         acceptBtn.Enabled = denyBtn.Enabled = sendBtn.Enabled = viewBtn.Enabled = false;
         this.onMsgFocusOut();
