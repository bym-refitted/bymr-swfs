package com.monsters.frontPage.categories
{
   ...
   public class Category
   {
      ...
      protected var _messages:Vector.<Message> = new Vector.<Message>();
      ...
      public function getNextQualifiedMessage() : Message
      {
         ...
         {
            _loc4_ = int(this._messages.indexOf(this.lastMessageSeen));
            if(_loc4_ >= 0)
            {
               _loc1_ = _loc4_ + 1;
         ...
         {
            _loc5_ = this._messages[_loc3_];
            if((!_loc5_.hasBeenSeen || this._doesViewRepeatedly) && _loc5_.areRequirementsMet)
            {
               return _loc5_;
            }
