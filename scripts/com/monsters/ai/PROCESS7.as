package com.monsters.ai
{
   ...
   public class PROCESS7 implements IPROCESS
   {
      ...
      public function ProcessC(param1:Solution) : void
      {
         ...
         {
            _loc22_ = param1.resourcesGained / (param1.damageTaken + param1.resourcesGained);
            if(_loc22_ < 0.5)
            {
               _loc22_ = 0.5;
            }
         ...
         var _loc11_:Number = BASE.BaseLevel().level / 40;
         if(_loc11_ < 0)
         {
            _loc11_ = 0;
         }
         ...
         var _loc12_:String = WMATTACK._looters[int((WMATTACK._looters.length - 2) * _loc11_) + 1];
         var _loc13_:String = WMATTACK._tanks[int((WMATTACK._tanks.length - 1) * _loc11_)];
         var _loc14_:Number = 0;
         var _loc15_:Object = {};
         var _loc16_:Number = GLOBAL._mapWidth * 0.25;
         var _loc18_:Number = Point.distance(param1.entryPoint,new Point(param1.targetBuilding.x,param1.targetBuilding.y));
         if(_loc18_ < 100)
         {
            _loc16_ += 100 - _loc18_;
         }
