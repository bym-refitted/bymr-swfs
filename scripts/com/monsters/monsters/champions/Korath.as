   ...
   public class Korath extends ChampionBase
   {
      ...
      override protected function doAttackDamage() : void
      {
         ...
         else
         {
            ...
            {
               ATTACK.Damage(_tmpPoint.x,_tmpPoint.y - 5,damage * 1 * (100 - (_targetBuilding._fortification.Get() * 10 + 10)) / 100,_mc.visible);
            }
            else
            {
               ATTACK.Damage(_tmpPoint.x,_tmpPoint.y - 5,damage * 1,_mc.visible);
            }
            if(_targetCreep)
            {
               _targetCreep.modifyHealth(-(damage * 1));
               this.addFlameDOT(_targetCreep);
            }
            else if(_targetBuilding)
            {
               _targetBuilding.modifyHealth(damage * 1,this);
            }
            else
            {
               findTarget();
      ...
      {
         var _loc3_:Point = Point.interpolate(_tmpPoint.add(new Point(0,-50)),_targetCreep._tmpPoint,0.8);
         var _loc4_:FIREBALL = FIREBALLS.Spawn2(_loc3_,_targetCreep._tmpPoint,_targetCreep,8,damage / 4,0,FIREBALLS.TYPE_MAGMA,1,this);
         _loc4_.addEventListener(FIREBALL.COLLIDED,this.addFlameDOTEvent,false,0,true);
      }
      ...
      private function quake(param1:int) : void
      {
         ...
         var _loc6_:G4QuakeGraphic = new G4QuakeGraphic(20,m_range * 2.5,BYMConfig.instance.RENDERER_ON ? new Point(_rasterPt.x,_rasterPt.y + _graphic.height * 0.6) : null);
...
class G4QuakeGraphic
{
   ...
   public function G4QuakeGraphic(param1:uint, param2:uint, param3:Point = null)
   {
      ...
      var _loc4_:GlowFilter = new GlowFilter(0xff6600,1,20,20,5 + Math.random() * 5,1,false,false);
