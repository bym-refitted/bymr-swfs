   ...
   public class Krallen extends ChampionBase
   {
      ...
      override public function findTarget(param1:int = 0) : void
      {
         ...
         if(!_loc17_)
         {
            ...
            {
               _loc15_ = _loc7_;
               if(_loc15_.health > 0 && (_loc15_._used > 0 || _loc15_._monstersDispatchedTotal > 0))
               {
                  _loc12_ = GRID.FromISO(_loc7_._mc.x,_loc7_._mc.y + _loc7_._middle);
                  _loc13_ = GLOBAL.QuickDistance(_loc11_,_loc12_) - _loc7_._middle;
                  _loc14_.push({
         ...
         else
         {
            ...
            else if(_movement == "fly")
            {
               ...
               {
                  _loc24_ = Math.atan2(_tmpPoint.y - _targetCenter.y,_tmpPoint.x - _targetCenter.x) * 57.2957795;
                  _loc24_ = _loc24_ + (Math.random() * 40 - 20);
                  _loc24_ = _loc24_ / (180 / Math.PI);
                  _loc25_ = 2 * 60 + Math.random() * 10;
