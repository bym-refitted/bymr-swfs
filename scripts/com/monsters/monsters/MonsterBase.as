package com.monsters.monsters
{
   ...
   public class MonsterBase extends GameObject implements IAttackable, IComponentOwner
   {
      ...
      public var _attackComponents:Vector.<Component> = new Vector.<Component>();
      ...
      override public function modifyHealth(param1:Number, param2:ITargetable = null) : Number
      {
         ...
         {
            _loc6_ = this._components[_loc4_];
            if(_loc6_ is IDefendingComponent)
            {
               param1 = IDefendingComponent(_loc6_).onDefend(this,param1,param2);
            }
            _loc4_++;
         }
         var _loc5_:Number = param1 + health;
         if(_loc5_ == health)
         {
            return 0;
         }
         ...
         if(k_DOES_PRINT_DETAILED_LOGGING)
         {
            ...
            print("[object MonsterBase] was modified for " + param1 + (!!(_loc3_ - param1) ? "(" + _loc3_ + " - " + (_loc3_ - param1) + ")" : "") + " health points, left with " + health + " out of " + maxHealth + "hp");
         }
      ...
      {
         var _loc2_:int = int(this.m_filters.indexOf(param1));
         if(_loc2_ >= 0)
         {
      ...
      public function findTarget(param1:int = 0) : void
      {
         ...
         else if(param1 == 4)
         {
            for each(_loc4_ in BASE._buildingsTowers)
            {
               ...
               {
                  _loc12_ = _loc4_;
                  if(_loc12_.health > 0 && (_loc12_._used > 0 || _loc12_._monstersDispatchedTotal > 0))
                  {
                     _loc9_ = GRID.FromISO(_loc4_._mc.x,_loc4_._mc.y + _loc4_._middle);
                     _loc10_ = GLOBAL.QuickDistance(_loc8_,_loc9_) - _loc4_._middle;
                     _loc11_.push({
            ...
            else if(this._movement == "fly" || this._movement == "fly_low")
            {
               ...
               if(this._creatureID == "IC5")
               {
                  if(!this._targetCreep)
                  {
                     ...
                     {
                        this._movement = "fly";
                        _loc20_ = Math.atan2(this._tmpPoint.y - this._targetCenter.y,this._tmpPoint.x - this._targetCenter.x) * 57.2957795;
