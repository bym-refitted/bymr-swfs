package com.monsters.inferno
{
   ...
   public class InfernoPopupInfoMine extends PopupInfoMine_CLIP
   {
      ...
      private var _popupMoveYard:InfernoPopupMoveYard;
      
      private var _scroller:ScrollSet;
      
      public function InfernoPopupInfoMine()
      {
         super();
         x = 455;
         y = 260;
         ...
         {
            InfernoMap._mc.ShowRelocateMePopup(_cell);
         });
         ...
         this._popupMoveYard = new InfernoPopupMoveYard();
         (mcFrame as frame).Setup();
      }
      
      private function StartTransferM(param1:MouseEvent) : *
      {
         if(this.bMonsters.Enabled && !InfernoMap._monsterTransferInProgress && !InfernoMap._resourceTransferInProgress && GLOBAL._mapOutpost.length > 0 && this._hasMonsters)
         {
            InfernoMap._mc.ShowMonstersA(this._cell);
         }
      }
      
      public function Hide(param1:MouseEvent = null) : *
      {
         InfernoMap._mc.HideInfoMine();
      }
      ...
      public function Setup(param1:InfernoMapCell) : *
      {
         ...
         GLOBAL._attackerCellsInRange = InfernoMap._mc.GetCellsInRange(this._cell.X,this._cell.Y,4 + _loc3_);
         ...
         {
            _loc8_ = 0;
            while(_loc8_ < InfernoMap._bookmarks.length)
            {
               if(InfernoMap._bookmarks[_loc8_].location.x == this._cell.X && InfernoMap._bookmarks[_loc8_].location.y == this._cell.Y)
               {
      ...
      public function PendingInvite() : *
      {
         ...
         InfernoMap.GetCell(this._cell.X,this._cell.Y,true);
      }
      ...
      public function ShowInviteMigrate() : *
      {
         ...
         InfernoMap._mc.addChild(this._message);
      ...
      public function Update() : *
      {
         ...
         var _loc4_:InfernoMapPopupInfoMonster = null;
         if(this._cell._updated)
         {
         ...
         if(this._cell._monsters)
         {
            ...
            for(_loc3_ in this._cell._monsters)
            {
               if(_loc1_ < 3)
               {
                  ...
                  {
                     _loc4_ = new InfernoMapPopupInfoMonster();
