package com.monsters.inferno
{
   ...
   public class InfernoMapCell extends MapRoomCell_CLIP
   {
      public var X:int;
      ...
      public function InfernoMapCell()
      {
         super();
      ...
      public function Update() : void
      {
         ...
         if(InfernoMap._viewOnly && this._baseID == InfernoMap._inviteBaseID)
         {
            mc.mcPrompt.tDesc.htmlText = KEYS.Get("map_relocateyard");
            ...
            {
               InfernoMap.PreAcceptInvitation(InfernoMap._mc as MovieClip);
            });
            mc.mcPrompt.bNo.addEventListener(MouseEvent.MOUSE_UP,InfernoMap.RejectInvitation);
         }
      }
      ...
      public function Tick(param1:int = 0) : Boolean
      {
         ...
         {
            mc.mcPlayer.mcWorker.visible = false;
            if(this._baseID == InfernoMap._inviteBaseID)
            {
               if(this._over)
               {
      ...
      public function TickHousing(param1:int = 0) : Boolean
      {
         var _loc3_:int = 0;
      ...
      private function Click(param1:MouseEvent) : *
      {
         ...
         if(Boolean(InfernoMap._mc) && InfernoMap._mc._dragged)
         {
            return;
         }
         if(InfernoMap._inviteBaseID == this._baseID)
         {
            return;
         }
         InfernoMap._currentPosition = new Point(this.X,this.Y);
         if(GLOBAL._local)
         {
         ...
         InfernoMap.TransferMonstersB(this);
         if(InfernoMap._viewOnly && this._base > 0)
         {
            InfernoMap._mc.ShowInfoViewOnly(this);
         }
         else if(this._base > 0 && !InfernoMap._monsterTransferInProgress)
         {
            if(this._mine)
            {
               InfernoMap._mc.ShowInfoMine(this);
            }
            else
            {
               InfernoMap._mc.ShowInfoEnemy(this);
            }
         }
      }
      ...
      private function SmokeAdd() : *
      {
         ...
         _loc1_.addChild(new Bitmap(InfernoMap._smokeBMD));
         _loc1_.x = -10;
         _loc1_.y = -90;
